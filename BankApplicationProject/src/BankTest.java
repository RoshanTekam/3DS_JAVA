...
public class BankTest {
	public static void main(String[] args) {
		...
		BankAccount bankAccount= bank.applyForJointBankAccount(bankAppForm2, bankAppForm3);
...
class Bank {
	...
	BankAccount applyForABankAccount(BankApplicationForm bankAppFormRef) {
		...
		bankAccObj.setAccountNumber(accNum);// real time, connect to the DB, and find out the last account number
											// generated, and one
	...
	BankAccount applyForJointBankAccount(BankApplicationForm applicationForm,BankApplicationForm applicationForm2)
	{
		System.out.println("Recieved the application from  : " + applicationForm.getApplicantName());
		System.out.println("Creating bank Account ");
		BankAccount bankAccObj = new BankAccount(); // a blank object
		
		double val=Math.random()*100;
		long accNum=(long)val;
		
		bankAccObj.setAccountNumber(accNum);
		bankAccObj.setAccountHolder(applicationForm.getApplicantName()+" "+applicationForm2.getApplicantName());
		bankAccObj.setAccountType("joint");
		bankAccObj.setIfscCode("SBIN0008");
		bankAccObj.setAccountBalance(10000);
		return bankAccObj;
	}
}
